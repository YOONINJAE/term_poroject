...
// Modified by Alex Park 10, Aug, 2019
// Tested with NUCLEO F401RE board
// This VL53L0x library was modified from that of the Arduino made by  Polou
...
// Record the current time to check an upcoming timeout against
#define startTimeout() (timeout_start_ms = _tmr.read_ms())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && (_tmr.read_ms() - timeout_start_ms) > io_timeout)
...
// Public Methods //////////////////////////////////////////////////////////////
void VL53L0X::setApplication(applicationType mode){
   if ( LONG_FAST || LONG_ACCURATE){
  ...
  if( LONG_FAST || SHORT_FAST) // reduce timing budget to 20 ms (default is about 33 ms) 
      setMeasurementTimingBudget(20000);  // minimum timing budget 20 ms
  else // increase timing budget to 200 ms
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  buffer[0] = reg;
  buffer[1] = value;
  last_status = _i2c.write(address, (const char *)buffer, 2, 0);
}
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  ...
  buffer[2]=value & 0xFF;
  last_status = _i2c.write(address, (const char *)buffer, 3, 0);
}
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  buffer[4] = value & 0xFF;   
  last_status = _i2c.write(address, (const char *)buffer, 5, 0);
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  uint8_t value;
  const char REG[] = {reg};
  _i2c.write(address, REG, 1,0);
  last_status = _i2c.read(address, (char*)&value,1,0);
  return value;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  const char REG[] = {reg};
  _i2c.write(address, REG, 1,0);
  last_status = _i2c.read(address, (char*)buffer,2,0);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  const char REG[] = {reg};
  _i2c.write(address, REG, 1,0);
  last_status = _i2c.read(address, (char*)buffer,4,0);
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
  ...
  for(int i=1;i<count+1;++i)
    buffer[i]=*(src+i-1);
  last_status = _i2c.write(address, (const char*)buffer, count+1, 0);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  const char REG[] = {reg};
  _i2c.write(address, REG, 1,0);
  last_status = _i2c.read(address, (char*)dst,count,0);
}
...
void VL53L0X::startContinuous(uint32_t period_ms)
{
  ...
  _int.fall(callback(this, &VL53L0X::dataReady)); 
}
...
uint16_t VL53L0X::readRange(){
    if(_dataRdy){
        _dataRdy=false;
        _range = readReg16Bit(RESULT_RANGE_STATUS + 10);
        writeReg(SYSTEM_INTERRUPT_CLEAR, 0x01);              
    } 
    return _range; 
}
